#import all the packages we will be needing for a program
...
#If the video argument is None(if we don't send any video file as input), let us start reading from the web cam
...
while True:
    ...
    #Now we have all the contours, let us loop over the contours
    for c in cnts:
        #if the contour is too small , ignore if
    ...
    cv2.putText(frame, "Motion Status : {}".format(text), (10,20), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.5, (0, 0, 255) ,2)
    cv2.putText(frame, datetime.datetime.now().strftime("%A %d %B %Y %I:%M:&S&p"), (10, frame.shape[0]-10),
    cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.35 , (0,0,255), 1)

    #Show the frame and the record if the user presses a key
